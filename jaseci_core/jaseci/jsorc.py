...
class JsOrc:
    ...
    def _svc(cls, service: str) -> CommonService:
        ...
        instance: cls.CommonService = instance["type"](config, manifest)
                            ...
                            for kind, confs in service.manifest.items():
                                for conf in confs:
                                    ...
                                    if kind == "Service":
                                        pod_name = name
                                    res = kube.read(
                                        kind, name, namespace=kube.resolve_namespace(conf)
                                    )
                                    if (
                                        hasattr(res, "status")
                                        and res.status == 404
                                ...
                                for to_be_removed in old_config_map.get(kind, {}).keys():
                                    res = kube.read(
                                        kind,
                                        to_be_removed,
    ...
    def db_regen(cls):
        ...
        if cls.db_check():
            try:
                dbrc = cls.settings("DB_REGEN_CONFIG")
                for item in kube.core.list_namespaced_pod(
                    namespace=kube.namespace, label_selector=f"pod={dbrc['pod']}"
                ).items:
                    kube.core.delete_namespaced_pod(
                        item["name"], kube.namespace
                    )
            except Exception:
